...
void deployAIModel(ServerNode* head, const char* ai_model_name, int required_vram) {
    ...
    temp = head; int single_node_id = -1;
    while (temp != NULL) {
        int free_vram = temp->vram_total - temp->vram_used;
        if (temp->status == 1 && free_vram >= required_vram) { single_node_id = temp->server_id; break; }
        temp = temp->next;
    }
    if (single_node_id != -1) {
        temp->vram_used += required_vram; 
        printf(GREEN "    [+] Cap phat thanh cong (Single-Node) tren may chu ID %d!\n" RESET, single_node_id);
    } else {
        printf(MAGENTA "    [*] Kich hoat phan tan (Tensor Parallelism) cho %s...\n" RESET, ai_model_name);
        temp = head; int vram_needed = required_vram;
        while (temp != NULL && vram_needed > 0) {
            int free_vram = temp->vram_total - temp->vram_used;
            if (temp->status == 1 && free_vram > 0) {
                int allocated = (free_vram >= vram_needed) ? vram_needed : free_vram;
                temp->vram_used += allocated; vram_needed -= allocated;
                printf("     -> Cắt %dGB sang Node %d\n", allocated, temp->server_id);
            }
            temp = temp->next;
        }
        printf(GREEN "    [+] Trien khai phan tan hoan tat!\n" RESET);
    }
}
